...
def evaluate(expression):
    ...
    if "+" in expression:
        ...
        answer = int(split[0]) + int(split[-1])
        return answer
    elif "-" in expression:
        split = expression.split("-")
        answer = int(split[0]) - int(split[-1])
...
text_result = tk.Text(root, height = 2, width = 42)
...
btn_minus = tk.Button(root, text = "-", command = lambda: add_to_calc("-"), width = 5)
btn_minus.grid(row = 3, column = 4)
btn_mult = tk.Button(root, text = "*", command = lambda: add_to_calc("*"), width = 5)
btn_mult.grid(row = 4, column = 4)
btn_divide = tk.Button(root, text = "/", command = lambda: add_to_calc("/"), width = 5)
btn_divide.grid(row = 5, column = 4)
btn_clear = tk.Button(root, text = "C", command = clear_field, width = 14)
btn_clear.grid(row = 6, column = 1, columnspan=2)
btn_equals = tk.Button(root, text = "=", command = evaluate_calc, width = 14)
